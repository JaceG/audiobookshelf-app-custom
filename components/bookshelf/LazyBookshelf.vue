  ...
  methods: {
    ...
    async fetchEntities(page) {
      ...
      if (this.$props.isPlaylist) {
        payload = await this.$nativeHttp.get(`/api/custom/libraries/${this.currentLibraryId}/${entityPath}`).catch((error) => {
          console.error('failed to fetch books', error)
          return null
        })
      } else {
        payload = await this.$nativeHttp.get(`/api/libraries/${this.currentLibraryId}/${entityPath}${fullQueryString}&personalProfile=${this.isPersonalLibrary}`).catch((error) => {
          console.error('failed to fetch books', error)
          return null
        })
      }
