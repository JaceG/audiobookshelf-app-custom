      ...
      <div class="relative z-10 px-3 py-4">
        ...
        <div class="col-span-full">
          <div v-if="showPlay || showRead" class="flex mt-4 -mx-1">
            ...
            <div v-if="isPlayable" class="flex mt-4 -mx-1 justify-center">
            ...
            <div v-else class="mx-auto">
              <ui-btn color="success" class="flex items-center justify-center flex-grow mx-1" @click="addToMyPlaylist">
...
export default {
  ...
  methods: {
    ...
    addToMyPlaylist() {
      const bookId = this.localLibraryItem?.id || this.libraryItem?.id
      this.$nativeHttp
        .post(`/api/custom/assign-audiobook`, { libId: bookId }, { connectTimeout: 5000 })
        .then((data) => {
          this.$toast.success(data?.message || `Audiobook added to playlist`)
          let selectedLibraries = this.$store.getters['user/selectedLibraries']
          selectedLibraries = [...selectedLibraries, bookId]
          this.$store.commit('user/selectedLibraries', selectedLibraries)
    ...
    init() {
      ...
      const selectedLibraries = this.$store.getters['user/selectedLibraries']
      const bookId = this.localLibraryItem?.id || this.libraryItem?.id
      this.isPlayable = Boolean(selectedLibraries.includes(bookId))
      this.windowWidth = window.innerWidth
